...
def panel_rotation(panels_series, solar_roof_area,new_image,pl, pw, l, w, solar_angle,high_reso_orig,fname):
    cnt=0
    cnt2=0
    ...
    for _ in range(panels_series - 2):
        print(str(_)+" out of loop 1 "+str(panels_series-2))
        for col in range(0, cols, l + 1):
            print(str(col) +"loop 2 out of "+str(cols))
            for row in range(0, rows, w + 1):
                print("loop 3 "+str(row)+" out of "+str(rows))
    ...
    # cv2.imshow(high_reso_new)
    print("cnt",cnt)
    print("cnt2",cnt2)
    result_2 = Image.fromarray(high_reso_new)
    result.paste(result,(0,0))
    # result_2.save("otpt.png",'PNG')
    plt.figure()
    plt.axis('off')
    plt.imshow(high_reso_orig)
    
    plt.savefig("firstapp/static/images/otpt.png", bbox_inches='tight')
    plt.figure()
    plt.axis('off')
...
def main1(fname):
    # images = glob.glob('static-map2.jpg')
    # latitude = ??
    ...
    pl, pw, l, w, solar_angle = 4, 1, 4, 2, 30
    
    image = cv2.imread(fname.path)
...
#     image = cv2.resize(image, dim, interpolation = cv2.INTER_AREA)
    ...
    return panel_rotation(pl, solar_roof,new_image,pl, pw, l, w, solar_angle,high_reso_orig,fname.path)
